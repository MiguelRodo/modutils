test_that("get_wald_stats works", {
  ...
  expect_identical(class(get_wald_stats(fit = mod_lm, var = '0'))[1],
                   "tbl_df")
  expect_identical(class(get_wald_stats(fit = mod_lmer, var = '0'))[1],
                   "tbl_df")
  expect_identical(class(get_wald_stats(fit = mod_glmer, var = '0'))[1],
                   "tbl_df")
  ...
  lm_tbl_x_wald <- get_wald_stats(fit = mod_lm_x, var = "x")
  ...
  lmer_tbl_x_wald <- get_wald_stats(fit = mod_lmer_x, var = "x")
  ...
  glmer_tbl_x_wald <- get_wald_stats(fit = mod_glmer_x, var = "x")
  ...
  lm_tbl_x_wald_1 <- get_wald_stats(fit = mod_lm_x_spline_df_1, var = "x",
                                  match_condn = 'any')
  expect_equal(lm_stat_x_lin, sqrt(lm_tbl_x_wald_1$stat))
  lm_tbl_x_wald_3 <- get_wald_stats(fit = mod_lm_x_spline_df_3, var = "x",
                                  match_condn = 'any')
  ...
  expect_error(get_wald_stats(fit = mod_lm_x_spline_df_1, var = "x",
                              match_condn = 'exact'))

  # check that we can match when we use non-alphanumeric characters
  expect_identical(lm_tbl_x_wald_3,
                   get_wald_stats(fit = mod_lm_x_spline_df_3, var = "splines::ns(x",
                                  match_condn = 'start'))
